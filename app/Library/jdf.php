...
class jdf{
    ...
    public function getPersianDate($miladi,$format)
    {
        date_default_timezone_set('UTC');
        $date = \DateTime::createFromFormat('Y-m-d H:i:s', $miladi);
        if($date == '')
            return '';
        return $this->jdate($format,mktime($date->format('H'),$date->format('i'),$date->format('s'),$date->format('m'),$date->format('d'),$date->format('Y')));
    }

    public function getPersianDate2($miladi,$format)
    {
        $date = \DateTime::createFromFormat('Y-m-d H:i:s', $miladi);
        if($date == '')
            return '';
    ...
    public function getUTCDate($jalali)
    {
        $date = explode('-', $jalali);

        $dateEn = $this->jalali_to_gregorian($date[0], $date[1], $date[2]);
        $dateEn = new \DateTime($dateEn[0].'-'.$dateEn[1].'-'.$dateEn[2]);
        return $dateEn->format("Y-m-d");
    }

    public function jalali_day($jalali_date,$mode)
    {
        $d = explode('-',trim($jalali_date));
        $jTog = $this->jalali_to_gregorian($d[0],$d[1],$d[2]);
        $dateTime  = \DateTime::createFromFormat('Y-m-d',$jTog[0].'-'.$jTog[1].'-'.$jTog[2]);
        $timestamp = mktime(00,00,00,$dateTime->format('m'),$dateTime->format('d'),$dateTime->format('Y'));
        if ($mode == 'timestamp')
            return $timestamp;
        return $this->jdate('l',$timestamp);
    }

    public function date_to_timestamp($date,$format)
    {
        $dateTime = \DateTime::createFromFormat($format,$date);
        $currentTimestamp = mktime($dateTime->format('H'),$dateTime->format('i'),$dateTime->format('s'),$dateTime->format('m'),$dateTime->format('d'),$dateTime->format('Y'));
        return $currentTimestamp;
    }

    public function date_diffrent($fromDate,$toDate) {
        $jdf = new jdf();
        $d1 = explode('-',trim($fromDate));
        $d2 = explode('-',trim($toDate));
